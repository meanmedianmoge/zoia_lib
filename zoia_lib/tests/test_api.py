...
class TestAPI(unittest.TestCase):
    ...
    def test_api_download_bin(self):
        ...
        self.assertIsNone(f, "Retrieved patch data for a patch that does not "
        ...
        # Try to pass in None.
        f = ps.download(None)
        self.assertIsNone(f, "Retrieved patch data without passing a patch "
                             "id.")
        # Try to actually download a .bin file.
        f = ps.download("105634")
        self.assertIsNotNone(f, "Did not retrieve patch data despite the"
                                " patch id existing in PatchStorage.")
        self.assertTrue(isinstance(f[0], bytes),
                        "Returned tuple did not contain binary data in the "
                        "first element.")
        ...
        try:
            ...
            self.fail("Returned tuple did not contain valid json data in the "
                      "second element.")

        try:
            validate(instance=f[1], schema=meta_schema)
        except ValueError:
            self.fail("Returned json data failed to validate against the "
                      "MetadataSchema.json schema.")

        self.assertTrue("id" in jf,
                        "Returned min item did not contain the id attribute.")
        self.assertTrue("link" in jf,
                        "Returned min item did not contain the link "
                        "attribute.")
        self.assertTrue("content" in jf,
                        "Returned min item did not contain the content "
                        "attribute.")
        self.assertTrue("files" in jf,
                        "Returned min item did not contain the files "
                        "attribute.")
        self.assertTrue("preview_url" in jf,
                        "Returned min item did not contain the preview_url "
                        "attribute.")
        self.assertTrue("revision" in jf,
                        "Returned min item did not contain the revision "
                        "attribute.")
        self.assertTrue("view_count" in jf,
                        "Returned min item did not contain the review_count "
                        "attribute.")
        self.assertTrue("like_count" in jf,
                        "Returned min item did not contain the like_count "
                        "attribute.")
        self.assertTrue("download_count" in jf,
                        "Returned min item did not contain the download_count "
                        "attribute.")
        self.assertTrue("author" in jf,
                        "Returned min item did not contain the author "
                        "attribute.")
        self.assertTrue("title" in jf,
                        "Returned min item did not contain the title "
                        "attribute.")
        self.assertTrue("created_at" in jf,
                        "Returned min item did not contain the created_at "
                        "attribute.")
        self.assertTrue("updated_at" in jf,
                        "Returned min item did not contain the updated_at "
                        "attribute.")
        self.assertTrue("tags" in jf,
                        "Returned min item did not contain the tags "
                        "attribute.")
        self.assertTrue("categories" in jf,
                        "Returned min item did not contain the categories "
                        "attribute.")
        self.assertTrue("state" in jf,
                        "Returned min item did not contain the state "
                        "attribute.")
        self.assertTrue("license" in jf,
                        "Returned min item did not contain the license "
                        "attribute.")
